...
| `learn_dagster/`       | A Python directory that contains code for your Dagster repository                 |
| `learn_dagster_tests/` | A Python directory that contains tests for `learn_dagster`                      |
...
pyenv virtualenv $PYTHON_VERSION learn_dagster
pyenv activate learn_dagster
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name learn_dagster python=PYTHON_VERSION
conda activate learn_dagster
```
...
Tests can be found in `learn_dagster_tests` and are run with the following command:

```bash
pytest learn_dagster_tests
```

As you create Dagster ops and graphs, add tests in `learn_dagster_tests/` to check that your
